...
module Dwt.Search (
    qGet
  , qGetLabLongErr, qGetLab
  , qGet1LongErr, qGet1 
  , qPutStLongErr, qPutSt
  , qRegexWord
) where
...
qGetSum :: RSLT -> QNode -> Either DwtErr [Node]
qGetSum = _qGetSum (\_ n -> n) nodes matchRel

qGetLab :: RSLT -> QNode -> Either DwtErr [LNode Expr]
qGetLab = _qGetSum f labNodes matchRelLab where
  f g n = (n, Mb.fromJust $ lab g n)

qGet1 :: RSLT -> QNode -> Either DwtErr Node
qGet1 g q = prependCaller "qGet1: " $ case qGetSum g q of
    Right [] -> Left (FoundNo, queryError, ".")
    Right [a] -> Right a
    Right as -> Left (FoundMany, queryError, ".")
    Left e -> Left e
