...
-- code
    ...
    type TotalDescendentsData = (S.Set Glab, S.Set Glab, PredMap, Graph)
      -- = (tdSnv, tdSv, undet, graph), described in long initial comment.

    relsIfNodeExists :: (Node -> S.Set Glab) -> Glab -> Graph -> S.Set Glab
      -- if glab is in gr, return its fs, where f is prds or sucs. 
    relsIfNodeExists f glab gr = 
    ...
    addToPredMap :: S.Set Glab -> PredMap -> Graph -> PredMap
      ...
      -- "increments the predMap" by adding more nodes whose prds must be checked for being totally descended from the ancestors of interest.
    ...
    visitUndetPrds  :: TotalDescendentsData -> TotalDescendentsData
      ...
      -- if the result is a pair (i, empty), then i is removed from undet and placed in tdSnv
    visitUndetPrds  (tdSnv,  tdSv, undet,  graph) =
              (tdSnv', tdSv, undet', graph)
      where td = S.union tdSnv tdSv
            swept = M.map (\set -> S.difference set td) undet
              -- where "sweep" = "remove parents determined totally descended from the ancestors of interest"
            (cleared, undet') = M.partition S.null swept
            tdSnv' = S.union tdSnv $ S.fromList $ M.keys cleared
              -- tdSnv should be empty, but its inclusion can't hurt
